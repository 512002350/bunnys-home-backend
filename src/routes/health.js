...
router.post('/update', async (req, res, next) => {
  try {
    ...
    if (heart_rate == null && steps == null && sleep_total == null) {
      return res.status(400).json({ error: '至少需要 heart_rate / steps / sleep_total 之一' });
    }

    const saved = await insertHealthData({
      heart_rate: heart_rate != null ? parseInt(heart_rate) : null,
      steps: steps != null ? parseInt(steps) : null,
      sleep_total: sleep_total != null ? parseInt(sleep_total) : null,
      sleep_deep: sleep_deep != null ? parseInt(sleep_deep) : null,
      sleep_light: sleep_light != null ? parseInt(sleep_light) : null,
